...
async function analyzeXtremeSales() {
    console.log('🔍 Analizando sitio 30x xtreme-sales...\n');
    
    try {
        // Descargar página xtreme-sales específicamente
        const html = await downloadContent('https://www.30x.org/xtreme-sales', '30x_xtreme_sales.html');
        ...
        fs.writeFileSync(path.join(outputDir, '30x_xtreme_styles.css'), inlineCSS);
        console.log('✓ Estilos inline extraídos de xtreme-sales');
        ...
        // Guardar análisis
        fs.writeFileSync(path.join(outputDir, '30x_xtreme_structure.json'), JSON.stringify(structure, null, 2));
        console.log('✓ Análisis de estructura xtreme-sales guardado');
        
        // Crear reporte específico
        const report = `
# Análisis del Sitio 30x Xtreme Sales - Reporte Técnico
...
${structure.headings.map(h => `- ${h}`).join('\n')}
        ...
        fs.writeFileSync(path.join(outputDir, '30x_xtreme_analysis_report.md'), report);
        console.log('✓ Reporte de análisis xtreme-sales creado');
